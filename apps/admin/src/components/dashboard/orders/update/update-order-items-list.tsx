...
// Hooks
import { useOrderDetails, useUpdateOrderItem } from '@/hooks/useOrder';

// Types/Utils
import { formatPrice } from '@/utils/price.util';
import { OrderResponse } from '@/hooks/useOrder/types';

interface UpdateOrderItemsListProps {
  orderId: string;
}
...
function UpdateOrderItemsList({ orderId }: UpdateOrderItemsListProps) {
  ...
  const handleEditItem = (item: OrderResponse['items'][number]) => {
    setEditingItem({
      ...
      price: item.price,
      vatPercentage: item.productId.vat,
      discountAmount: item.discount,
      bulkDiscountAmount: item.bulkDiscount,
                    ...
                    <div className="flex-1 space-y-4">
                      <div className="flex items-start justify-between">
                        ...
                        <Button
                          ...
                          className="hover:bg-[var(--baladi-primary)]/10 flex items-center gap-1 text-xs text-[var(--baladi-primary)] transition-colors hover:text-[var(--baladi-primary-dark)]"
                        >
                          <Edit size={12} />
                          Rediger produkt
                        </Button>
                            ...
                            {item.bulkDiscount > 0 && (
                              <div className="flex items-center justify-between">
                                ...
                                <span className="font-[family-name:var(--font-dm-sans)] font-semibold text-green-600">
                                  -
                                  {formatPrice(
                                    item.bulkDiscount * item.quantity,
                                  )}{' '}
                                  kr
                                </span>
                              </div>
...
export default memo(UpdateOrderItemsList);
